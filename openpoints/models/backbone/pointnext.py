...
class PointNextDecoder(nn.Module):
    def __init__(self,
        ...
        for i in range(-1, -n_decoder_stages, -1):
            decoder[i] = self._make_dec(
                skip_channels[i], fp_channels[i])
        decoder[-n_decoder_stages] = self._make_dec(
                skip_channels[-n_decoder_stages], fp_channels[-n_decoder_stages], act_args=None) # the last layer of decoder without active function
    ...
    def _make_dec(self, skip_channels, fp_channels, act_args={'act': 'relu'}):
        layers = []
        mlp = [skip_channels + self.in_channels] + \
              [fp_channels] * self.decoder_layers
        layers.append(FeaturePropogation(mlp, act_args=act_args))
