...
class PointTransformerLayer(nn.Module):
    def __init__(self, in_planes, out_planes, share_planes=8, nsample=16):
        ...
        '''
        group_args={'NAME': 'ballquery', 'radius': 0.1, 'nsample': nsample}
        self.grouper = create_grouper(group_args)
        self.mid_channel = 8
        norm_args={'norm': 'bn1d'}
        act_args={'act': 'relu'}
        conv_args={'order': 'conv-norm-act'}
        self.posencoder = create_convblock2d(3, in_planes, norm_args=norm_args, act_args=None, **conv_args)
        self.att_k = create_convblock1d(in_planes, out_planes // self.mid_channel, norm_args=None, act_args=None)
        self.att_q = create_convblock1d(in_planes, out_planes // self.mid_channel, norm_args=None, act_args=None)
        self.att_v = create_convblock2d(in_planes, out_planes, norm_args=norm_args, act_args=act_args)
        self.actfunc = create_act(act_args)
        self.att_vec = nn.Sequential(nn.BatchNorm2d(out_planes // self.mid_channel), self.actfunc, nn.Conv2d(out_planes // self.mid_channel, out_planes // self.mid_channel, kernel_size=1), nn.BatchNorm2d(out_planes // self.mid_channel), self.actfunc, nn.Conv2d(out_planes // self.mid_channel, out_planes // self.mid_channel, kernel_size=1))
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        p_r, x_k = self.grouper(p, p, x_k.transpose(1,2))
        _, x_v = self.grouper(p, p, x_v.transpose(1,2))
        ...
        set_trace()
        '''
        x_k = x_k.transpose(2,3).transpose(1,2)
        x_v = x_v.transpose(2,3).transpose(1,2)
        p_r = p_r.transpose(2,3).transpose(1,2)
...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            x = self.relu(self.bn(self.linear(x).transpose(1,2)).transpose(1,2))  # (n, c)
        return [p, x, o]
