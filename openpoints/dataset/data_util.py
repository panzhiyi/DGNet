...
def voxelize(coord, mask=None, voxel_size=0.05, hash_type='fnv', mode=0):
...
def voxelize_sparse(coord, mask, voxel_size=0.05, hash_type='fnv', mode=0):
    discrete_coord = np.floor(coord / np.array(voxel_size))
    if hash_type == 'ravel':
        key = ravel_hash_vec(discrete_coord)
    else:
    ...
    _, voxel_idx, count = np.unique(key_sort, return_counts=True, return_inverse=True)
    if mode == 0:  # train mode
        num_label = np.cumsum(mask_sort)[np.cumsum(count)-1]-np.insert(np.cumsum(mask_sort)[np.cumsum(count)-1], 0, 0)[0:-1]
...
def crop_pc_sparse(coord, feat, label, split='train',
            voxel_size=0.04, voxel_max=None,
    ...
    if voxel_size and downsample:
        ...
        mask = mask.astype(np.int16)
        uniq_idx = voxelize_sparse(coord, mask, voxel_size)
        coord, feat, label, mask = coord[uniq_idx], feat[uniq_idx] if feat is not None else None, label[uniq_idx] if label is not None else None, mask[uniq_idx] if mask is not None else None
    if voxel_max is not None:
        crop_idx = None
        N = len(label)  # the number of points
        mask_idx = np.arange(N)[mask==1]
        L = mask_idx.shape[0]
        if N >= voxel_max:
            if L >= voxel_max:
                init_idx = np.random.choice(L, voxel_max, replace=False) if 'train' in split else np.arange(voxel_max)
                crop_idx = mask_idx[init_idx]
            else:
                nomask_idx = np.arange(N)[mask==0]
                init_idx = np.random.choice(N - L, voxel_max - L, replace=False) if 'train' in split else np.arange(voxel_max - L)
                crop_idx = np.hstack([nomask_idx[init_idx], mask_idx])
            '''
            init_idx = np.random.randint(L) if 'train' in split else L // 2
            crop_idx = np.argsort(
                np.sum(np.square(coord - coord[mask_idx[init_idx]]), 1))[:voxel_max]
            '''
        elif not variable:
            # fill more points for non-variable case (batched data)
            query_inds = np.arange(N)
            padding_choice = np.random.choice(N, voxel_max - N)
            crop_idx = np.hstack([query_inds, query_inds[padding_choice]])
        crop_idx = np.arange(coord.shape[0]) if crop_idx is None else crop_idx
        if shuffle:
            shuffle_choice = np.random.permutation(np.arange(len(crop_idx)))
